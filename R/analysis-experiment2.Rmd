...
```{r}
# fn <- here("results", paste("results-", subj, ".csv", sep=""))
# data <- read.csv(fn, sep=",") %>% as_tibble()
```

```{r, message=FALSE}
# for testing analysis script
subjects = c("britta", "vini")
data <- tibble()
for(subj in subjects) {
  fn <- here("results", "pretests", paste("results-", subj, ".csv", sep=""))
  data_subj <- read.csv(fn, sep=",") %>% as_tibble() 
  data <- bind_rows(data, data_subj %>% add_column(submission_id = subj))
}
data <- data %>% group_by(submission_id) %>% 
   mutate(selection = case_when(
     length(str_split(response, "_")[[1]]) == 3 ~ "all",
     str_detect(response, "contrast") ~ "contrast", 
     str_detect(response, "_") ~ "both",
     T ~ response))
```
...
ids_out = attention_checks %>% filter(expected != selected_pic) %>% 
...
data.test = data %>%
  filter(trial_name %in% c("image_selection_01", "image_selection_02")) 
```

Number of times participants selected contrast picture (possibly among others)

```{r}
data.test %>% filter(str_detect(selected_pic, "contrast")) %>% 
  group_by(submission_id) %>% dplyr::count()
```
...
df <- data %>% filter(id %in% trials_lit_prag) %>% 
  dplyr::select(submission_id, response, type, id, selection)
...
# by trial
data.critical %>% group_by(picPair.short, QUD, response) %>% 
  summarize(n=n(), .groups = "drop_last") %>% 
  mutate(ratio = n / sum(n)) %>% 
  ggplot(aes(fill=response, y=QUD, x=ratio)) + 
  geom_bar(stat="identity", position=position_dodge(preserve = 'single')) +
  facet_wrap(~picPair.short, scales = "free")
```

Look at participants' responses to questions shown in the end

```{r}
data %>% select(submission_id, check_ann) %>% distinct()
data %>% select(submission_id, check_behavior) %>% distinct()
```
...
#   group_by(picPair, QUD) %>% mutate(n=n()) %>% 
...
df.critical = data.critical %>% 
  dplyr::select(submission_id, picPair, picPair.short, QUD, response) %>% 
  group_by(picPair, QUD)

bootstrap_samples = group_map(df.critical, function(df, df.group) {
  bootstrap = map(seq(1, N), function(i){
    tibble(response = sample(df$response, n, replace=T)) %>%
      group_by(response) %>% summarize(n=n()) %>%
      mutate(n = n/sum(n)) %>% add_column(iter=i) %>% rename(ratio = n)
    }) %>% bind_rows() %>% 
    add_column(picPair = df.group$picPair, QUD = df.group$QUD) %>% 
    group_by(response)
  
  bootstrap %>% mutate(ci_low = case_when(iter == CI_bounds[["low"]] ~ T, 
                                          T ~ F), 
                       ci_up = case_when(iter == CI_bounds[["up"]] ~ T, 
                                         T ~ F)) 
}) %>% bind_rows() %>% group_by(picPair, QUD)

CI.low = bootstrap_samples %>% filter(ci_low) %>% select(-iter, -ci_low, -ci_up) %>% 
  rename(ci.low = ratio)
CI.up = bootstrap_samples %>% filter(ci_up) %>% select(-iter, -ci_low, -ci_up) %>% 
  rename(ci.up = ratio)
...
data.critical.rate = df.critical %>% 
  group_by(picPair, picPair.short, QUD, response) %>% 
  summarize(n=n(), .groups = "drop_last") %>% 
  mutate(ratio = n/sum(n))
...
p
ggsave(here("results", "figs", "selection-rates.pdf"), p, width=7, height=4)
```
...
ggsave(here("results", "figs", "main-effect-question.pdf"), p.mainquestion,
       width=7, height=4)
```
...
p
ggsave(here("results", "figs", "empirical-responses.pdf"), p, width=7, height=4)
```
...
p
ggsave(here("results", "figs", "empirical-responses-rates.pdf"), p, width=7, height=4)
```
...
p.exh
ggsave(here("results", "figs", "main-effect-exhaustive.pdf"), p.exh,
       width=7, height=4)
```
...
p.nonExh
ggsave(here("results", "figs", "main-effect-nonExhaustive.pdf"), p.exh,
       width=7, height=4)
```
...
p.picPair
ggsave(here("results", "figs", "main-effect-picPair.pdf"), p.picPair,
       width=7, height=4)
```
...
bootstrap_RTs = group_map(critical.RT, function(df, df.group){
  bootstrap = map(seq(1, N), function(i){
    tibble(mean_RT = mean(sample(df$RT, n, replace=T)))
  }) %>% bind_rows() %>% 
...
p <- df.rt_means %>% ggplot(aes(x=exh_situation, y=mean_rt, color=nonExh_situation)) +
  ...
  annotate("text", x=rep("w/oDistractor", 2), y=c(13500, 15500), 
                label=c("A", "B"), hjust=-2)
p 
ggsave(here("results", "figs", "reaction-times-233.pdf"), p, width=7, height=4)
...
Reaction time is longer when at least one external picture is shown
...
hypothesis(fit_rt, "picPairC + picPairC:questionifp > 0")
hypothesis(fit_rt, "picPairD + picPairD:questionifp > 0")

# question: willq
hypothesis(fit_rt, "picPairB + picPairB:questionwillq > 0")
hypothesis(fit_rt, "picPairC + picPairC:questionwillq > 0")
...
p.mainpicpair = coeffs_rt_picPair %>% 
  ...
  scale_x_discrete(labels = c(B = parse(text=TeX("$\\hat{\\beta}_{B}$")), 
                              ...
                              A = parse(text=TeX("$\\hat{\\beta}_{A}$"))))
p.mainpicpair
ggsave(here("results", "figs", "main-effect-picPair-rts.pdf"), p.mainpicpair,
       width=7, height=4)
