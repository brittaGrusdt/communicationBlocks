...
save_plots = FALSE
target_dir = here("results", "experiment1")
...
p
if(save_plots) ggsave(file.path(plot_dir, "selection-rates.pdf"), p, width=7, height=4)
```
...
```{r, results='hide'}
model_path = here(target_dir, "brms-model-newlabels.rds")
...
p
if(save_plots) ggsave(file.path(plot_dir, "empirical-responses.pdf"), p, width=7, height=4)
```
...
p
if(save_plots) ggsave(file.path(plot_dir, "empirical-responses-rates.pdf"), p, width=7, height=4)
```
...
means = coeffs.exh %>% 
  summarize(mean_exhDistr = mean(exhDistr), 
            `mean_exhNoDistr` = mean(`exhNoDistr`))
...
p.exh
if(save_plots) ggsave(file.path(plot_dir, "main-effect-exhaustive.pdf"), p.exh, width=7, height=4)
```
...
p.nonExh
if(save_plots) ggsave(file.path(plot_dir, "main-effect-nonExhaustive.pdf"),
...
p.picPair
if(save_plots) ggsave(file.path(plot_dir, "main-effect-picPair.pdf"), 
...
if(save_plots) ggsave(file.path(plot_dir, "empiric-rts.pdf"), p.rt, width=7, height=4)
```
...
p <- df.rt_means %>% ggplot(aes(x=exh_situation, y=mean_rt, color=nonExh_situation)) +
  ...
  annotate("text", x=rep("w/oDistractor", 2), y=c(13500, 15500), 
                label=c("A", "B"), hjust=-2)
p 
if(save_plots) ggsave(file.path(plot_dir, "reaction-times-233.pdf"), p, width=7, height=4)
```
...
levels(df.rt_means_with_CIs$QUD) = c(neutral = "neutral", ifp = "if-p",
                                     willq = "will-q")
...
p 
if(save_plots) ggsave(file.path(plot_dir, "reaction-times-233-with-errorbars-plain.pdf"), p, width=7, height=4)

```

When the non-exhaustive picture is external (B + D, green) participants seem to 
respond more slowly than when the non-exhaustive picture is internal 
(A + C, orange), when question=ifp or question=willq.

# Reaction Times model 3x4 Design (QUD x picPair)
...
```{r, include=FALSE}
# question: neutral
hypothesis(fit_rt, "picPairB > 0") # IE
hypothesis(fit_rt, "picPairC > 0") # EI
hypothesis(fit_rt, "picPairD > 0") # EE

# question: ifp
hypothesis(fit_rt, "picPairB + picPairB:questionifp > 0")
hypothesis(fit_rt, "picPairC + picPairC:questionifp > 0") # posterior 0.44
hypothesis(fit_rt, "picPairD + picPairD:questionifp > 0")

# question: willq
hypothesis(fit_rt, "picPairB + picPairB:questionwillq > 0")
hypothesis(fit_rt, "picPairC + picPairC:questionwillq > 0") # posterior 0.65
hypothesis(fit_rt, "picPairD + picPairD:questionwillq > 0")
```
...
p.mainpicpair = coeffs_rt_picPair %>% 
  ...
  scale_x_discrete(labels = c(B = parse(text=TeX("$\\hat{\\beta}_{B}$")), 
                              ...
                              A = parse(text=TeX("$\\hat{\\beta}_{A}$"))))
p.mainpicpair
if(save_plots) ggsave(file.path(plot_dir, "main-effect-picPair-rts.pdf"),
...
1.-4. Reaction time is shorter for non-exhaustive internal than for nonExhaustive external when QUD=ifp or QUD=willq,  when QUD=neutral??
Intercept: exh=withDistractor, nonExh=external, question=neutral
...
if(file.exists(model_path)) {
  ...
  saveRDS(fit_rt_233, file=model_path)
}

# 1. exhaustive:without Distractor
# 1.a QUD: ifp
hypothesis(fit_rt_233, "nonExhinternal + exhwDoDistractor:nonExhinternal + nonExhinternal:questionifp + exhwDoDistractor:nonExhinternal:questionifp < 0")
# 1.b QUD: willq
hypothesis(fit_rt_233, "nonExhinternal + exhwDoDistractor:nonExhinternal + nonExhinternal:questionwillq + exhwDoDistractor:nonExhinternal:questionwillq < 0")
# 1.c QUD: neutral
hypothesis(fit_rt_233, "nonExhinternal + exhwDoDistractor:nonExhinternal + exhwDoDistractor:nonExhinternal < 0")


# 2. exhaustive: with Distractor
# 2.a QUD: ifp
hypothesis(fit_rt_233, "nonExhinternal + nonExhinternal:questionifp + nonExhinternal:questionifp < 0")
# 2.b QUD: willq
hypothesis(fit_rt_233, "nonExhinternal + nonExhinternal:questionwillq + nonExhinternal:questionwillq < 0")
# 2.c QUD: neutral
hypothesis(fit_rt_233, "nonExhinternal < 0")
...
samples = posterior_draws %>% 
  mutate(
    ...
    internal_ifp_woD = b_Intercept + b_nonExhinternal + b_questionifp + b_exhwDoDistractor + 
      `b_exhwDoDistractor:questionifp` + `b_exhwDoDistractor:nonExhinternal` +
      `b_nonExhinternal:questionifp` + `b_exhwDoDistractor:nonExhinternal:questionifp`,
...
samples %>% 
  transmute(internal_wDist_neutral = internal_neutral_withD < external_neutral_withD,
            internal_woD_neutral = internal_neutral_woD < external_neutral_woD
            ) %>% 
  summarize(internal_wDist_neutral.mean = mean(internal_wDist_neutral),
            internal_woD_neutral.mean = mean(internal_woD_neutral))
  
coeffs_QUD = samples %>% 
  transmute(nonExhinternal_neutral = (internal_neutral_withD + internal_neutral_woD)/2, 
            nonExhinternal_ifp = (internal_ifp_withD + internal_ifp_woD)/2,
            nonExhinternal_willq = (internal_willq_withD + internal_willq_woD)/2,
            
            nonExhexternal_neutral = (external_neutral_withD + external_neutral_woD)/2, 
            nonExhexternal_ifp = (external_ifp_withD + external_ifp_woD)/2,
            nonExhexternal_willq = (external_willq_withD + external_willq_woD)/2
            ) %>% 
  mutate(internal_smaller_neutral = nonExhinternal_neutral < nonExhexternal_neutral,
         internal_smaller_ifp = nonExhinternal_ifp < nonExhexternal_ifp,
         internal_smaller_willq = nonExhinternal_willq < nonExhexternal_willq)

mean(coeffs_QUD$internal_smaller_ifp)
mean(coeffs_QUD$internal_smaller_willq)
mean(coeffs_QUD$internal_smaller_neutral)
...
means = coeffs %>% summarize(mean_nonExhexternal = mean(nonExhexternal),
                             mean_nonExhinternal = mean(nonExhinternal))
