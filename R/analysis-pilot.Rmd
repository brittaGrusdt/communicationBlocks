...
df = data.critical %>% 
  ...
  mutate(pics = as.factor(pics), question = as.factor(question)) %>%
  rename(submissionID = submission_id)

fit <-
  brm(data = df,
      family = bernoulli,
      y ~ 0 + Intercept + pics * question + (1 + pics|submissionID) +
        (1|question),
      seed = 1)
fixef(fit)
pp_check(fit)
...
hypothesis(fit, "questionif_ant < Intercept", class = "b")
hypothesis(fit, "picsD > Intercept", class = "b")
hypothesis(fit, "picsC < Intercept", class = "b")
```

Just look at data for picture_combination with largest difference between question-condititions. (if1_un vs. if2_unu)

```{r, results='hide'}
fit2 <- 
  brm(data = df,
    family = bernoulli,
    y ~ 1 + question + (1 + pics | submissionID),
    seed = 1)
